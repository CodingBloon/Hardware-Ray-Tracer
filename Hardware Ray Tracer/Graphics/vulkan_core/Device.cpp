...
Core::Device::Device(Window* window) : window(window) {
	createInstance();
	setupDebugMessenger();
	createSurface();
	pickPhysicalDevice();
...
VkFormat Core::Device::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
void Core::Device::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer* buffer, VkDeviceMemory* bufferMemory) {
...
VkDeviceAddress Core::Device::getBufferDeviceAddress(VkBuffer buffer) {
...
VkCommandBuffer Core::Device::beginSingleTimeCommands() {
...
void Core::Device::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void Core::Device::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void Core::Device::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, uint32_t layerCount) {
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Core::Device::createSurface() {
...
void Core::Device::pickPhysicalDevice() {
	...
	for (const auto& device : devices) {
		if (isDeviceSuitable(device)) {
			int score = physicalDeviceScore(device);
			if (score > highest) {
...
void Core::Device::createLogicalDevice() {
	...
	VkPhysicalDeviceRayTracingPipelineFeaturesKHR rayTracingPipelineFeature{ VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR };
	rayTracingPipelineFeature.rayTracingPipeline = VK_TRUE;
	bufferDeviceAddressFeature.pNext = &rayTracingPipelineFeature;
	...
	deviceFeatures2.pNext = &bufferDeviceAddressFeature;
...
void Core::Device::createCommandPool() {
...
std::vector<const char*> Core::Device::getRequiredExtensions() {
...
void Core::Device::hasGflwRequiredInstanceExtensions() {
...
int Core::Device::physicalDeviceScore(VkPhysicalDevice device) {
